export { default as NavbarComponent } from "./Navbar/NavbarComponent";
export { default as FormPersonal } from "./FormPersonal/FormComponent";
export { default as FormWeeklyReport } from "./FormWeeklyReport/FormComponent";
export { default as ReportComponent } from "./Report/ReportComponent";
export { default as FormMultiplePersonal } from "./FormMultiplePersonal/FormComponent";
export { default as FormBasicDetails } from "./FormBasicDetails/FormComponent";
export { default as ExcelToJsonWeeklyForm } from "./ExcelToJsonWeeklyForm/FormComponent";
export { default as Footer } from "./Footer/FooterComponent";
